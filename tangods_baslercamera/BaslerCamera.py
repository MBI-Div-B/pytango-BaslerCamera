...
class BaslerCamera(Device):
    ...
    exposure = attribute(
        label="exposure (recommended 500)",
        dtype="float",
    ...
    def init_device(self):
        ...
        try:
            ...
            print(
                "Init was done to camera with serial: {:s}".format(
    ...
    def delete_device(self):
        try:
            print(
                "Close connection to camera with serial: {:s}".format(
                    self.device.GetSerialNumber()
                )
            )
            self.stop_grabbing()
